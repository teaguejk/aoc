...
pub fn parse(path: &str) -> Input {
    let lines = read_to_string(path).expect("error: could not read input file");

    let (rules, messages): (Vec<String>, Vec<String>) = lines
        .split_once("\n\n")
        .map(|(r, m)| {
            (
...
pub fn part1(input: &Input) -> i32 {
    ...
    valid.iter().map(|message| message[message.len() / 2]).sum()
}
...
pub fn part2(input: &Input) -> i32 {
    ...
    for message in messages {
        ...
        // apply sorting rules to a copy of the message if it fails
        let mut new_message = message.clone();

        for _ in 0..message.len() {
            for (before, after) in rules {
                if let Some(before_index) = new_message.iter().position(|x| x == before) {
                    if let Some(after_index) = new_message.iter().position(|x| x == after) {
                        if before_index > after_index {
                            pass = false;
                            new_message.swap(before_index, after_index);
                        }
                    }
                }
            }
        ...
        // push the new vec into the list
        if !pass {
            valid.push(new_message);
        }
    }
